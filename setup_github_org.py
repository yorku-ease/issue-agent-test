...
Usage: GITHUB_TOKEN=<token> python setup_github_org.py
Target org: yorku-ease
"""
import os
import sys
import time
...
# -- 1. Set org owner
OWNER = "yorku-ease"
print(f"Targeting org: {OWNER}")

# ── 2. Get or create GitHub repo ──────────────────────────────────────────
print(f"\nChecking repo {OWNER}/{REPO}...")
try:
    repo_data = api("post", "/orgs/yorku-ease/repos", json={
        "name": REPO,
...
print("\nChecking remote...")
try:
    run(f'git remote add org "{REPO_URL}"', cwd=cwd)
except Exception:
    run(f'git remote set-url org "{REPO_URL}"', cwd=cwd)
run("git branch -M main", cwd=cwd)
try:
    run("git push -u org main", cwd=cwd)
...
for i, branch_def in enumerate(BRANCHES):
    ...
    run(f"git checkout -B {branch} main", cwd=cwd)
    ...
    run(f"git push org {branch}", cwd=cwd)
